...
| `discovery.concurrency?` | 4 | maximum connector discovery tasks in flight during `search_tools`, `describe_tools`, in-sandbox discovery, `list_connectors` (including live probes), and operator UI catalog loading. A missing or non-positive/non-whole value uses the default |
| `discovery.catalogTtlSeconds?` | 300 | fresh TTL for cached tool lists |
| `discovery.persistCatalog?` | true | persist dynamically listed serializable catalogs as a manifest plus ≤1 MiB revision-addressed chunks. These catalogs are bounded at 100,000 tools and 32 MiB serialized; larger listings warn and fail without truncation ([format and rollover](./storage-and-credentials.md#persisted-tool-catalogs)) |
...
| `calls.maxBatchResultBytes?` | 100_000 | independent cap on the complete serialized `batch_call` envelope; an oversized envelope is stashed for `get_result` and replaced inline by ordered success/failure summaries plus the page handle. Per-connector child caps do not raise it; invalid values warn and fall back |
...
- `npm run typecheck` — `tsc --noEmit`, including the separate strict
  DOM-targeted browser project.
- `npm run build:operator-ui` — bundles the browser TypeScript and CSS into the
  committed embedded asset; `npm run check:operator-ui` fails when that asset
  is stale.
- `npm run test` — `vitest run` (both projects below).
- `npm run test:browser:install` — installs the Chromium binary used by the
  local browser suite (once per Playwright version).
- `npm run test:browser` — runs the focused operator flows in a real headless
  Chromium browser. This documented local runner is intentionally separate from
  `npm run check`; it covers the embedded bundle without adding a browser
  download to both CI Node-version jobs.
...
| Suite | Covers |
| --- | --- |
| `registry.test.ts` | id validation, duplicate rejection, address resolution, request-local cold-load coalescing, complete-catalog fingerprints and persistence decisions, manifest/chunk round trips with bounded-parallel chunk reads/writes, manifest withholding after any parallel chunk-write failure, torn-read and fingerprint-mismatch rejection, catalog size bounds, legacy-format rollover, tool-cache TTL/invalidation, broken-connector isolation, and an in-flight pre-credential-change refresh being discarded rather than republished after invalidation |
| `meta-tools.test.ts` | the registry-backed meta-tools: bounded/timed health and catalog fan-out (including best-effort probe-scope teardown that cannot replace the result or wait forever), ranked/paginated discovery with handler-side page/address maxima and a UTF-8 generated-result ceiling (including 100,000-tool page boundaries, duplicate addresses, multibyte text, and compact + JSON schemas), concise/full descriptions, MCP/value result modes, structured errors, OAuth flow, fields selection, truncation + paging (including what the cap measures for non-text content, and `get_result`'s offset validation and character-boundary alignment), schema-valid results for returns JSON cannot represent, batch parallelism/isolation, and catalog-lookup health accounting (a failing catalog counts call-for-call with a failing execution, a typed `auth_required` keeps its code, recovery clears the count, cache hits record nothing) |
...
| `server-route-contracts.test.ts` | the route contracts the `server.ts` extraction (#148) must keep byte-identical, pinned before any file moves: every built-in route answered ahead of connector-owned routes inside the security-header wrapper, open data-free operator shells with framing denied, per-route auth/same-origin/Clerk-only requirements with exact 401/403/405 bodies (including method-agnostic `/mcp` auth), auth-before-toolkit-resolution with byte-identical refusals for real and invented names, the single restricted-operator 403 across deployment-wide surfaces, `ScopedRegistry` visibility for an admitted toolkit, and OAuth callback `verifyState`-before-`finishAuth` ordering with one opaque refusal for every unverifiable callback |
...
| `ui.test.ts` | the shared data-free shells at `/`, `/credentials`, and `/activity` (direct loads, active links, page titles, manual-token fallback, Clerk sign-in including the absolute-https gate every `uiAuth` URL passes, MCP URL derivation), permanent `/ui` redirect, the read-only toolkit projection and its absence from the open shell, credential/activity/OAuth capability states, gated `/ui/data` with broken-connector isolation, bounded catalog/status fan-out, and bounded best-effort scope teardown, Credentials-only controls and the credential API including same-origin/bearer rejection, same-origin Clerk-only OAuth disconnect/reconnect, both stored-shape drift directions and replacement recovery, a stored superset (named and single) staying configured/testable with a non-blocking notice naming the leftover field, the liveness verdict Test records (and PUT/DELETE clear), `/ui/activity` paging, gate, read-time friendly actor labels and stable-ID fallback, connector/activity filtering, favicons, and OAuth result pages returning to `/` |
| `browser/operator-ui.spec.ts` | local Playwright Chromium coverage of the running embedded browser bundle: the open data-free shell becoming authenticated only after the private `/ui/data` request, credential add/test/replace/remove, downstream OAuth disconnect/restart, and Activity navigation through the private paged route |
...
| `call-admission.test.ts` | connector-scoped per-runtime downstream admission (Node + Workers) — concurrent batch bounds with input-order preservation, direct/code-mode and toolkit-view contention on one base-registry limiter, independent partitions, exact rolling-window reset and retry, queued cancellation without budget consumption, already-cancelled calls avoiding dispatch, active cancellation avoiding retry/health poisoning, bounded/contained partition keys, local-refusal health isolation, and payload-free `/health` aggregates |
