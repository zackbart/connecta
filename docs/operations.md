...
| `discovery.probeTimeoutMs?` | 30_000 | per-connector deadline for `list_connectors` probes and the catalog fan-out behind `search_tools`/`describe_tools`. Bounds the caller-facing wait only; it does not apply to tool calls or currently abort the underlying fetch |
