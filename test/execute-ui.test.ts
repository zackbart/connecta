...
describe("EmitCollector UI validation (U1)", () => {
  ...
  it("rejects missing or extra arguments, accepting nothing", () => {
    for (const args of [[], [VIEW, {}], [VIEW, "extra", true]]) {
      const sink = collector();
      expect(() => sink.acceptUi(...args)).toThrowError(
...
describe("the Apps shell (U5, U6)", () => {
  it("is a structurally valid HTML5 document at a versioned address", () => {
    expect(PROGRAM_UI_RESOURCE_URI).toBe("ui://connecta/program-ui/v3");
  ...
  it("speaks the Apps lifecycle and exposes no payload-to-host channel", () => {
    for (const method of [
      "ui/initialize",
    ...
    expect(PROGRAM_UI_SHELL_HTML).not.toContain('method: "tools/call"');
    expect(PROGRAM_UI_SHELL_HTML).not.toContain("connecta/read");
    expect(PROGRAM_UI_SHELL_HTML).not.toContain("view.contentWindow.postMessage");
