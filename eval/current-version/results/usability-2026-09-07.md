...
The execute instructions state the actual search, call, and batch result shapes,
recommend JSON schemas for programmatic inspection, and require checking batch
success before interpreting data. Failed checks and missing fields remain
...
The baseline is commit `bf7d96e04ffa9f910fd8e52f926253b0f838362c`.
...
- Final candidate: `2f1a29b4ef66a786cde5595bd339a116f68ae3f18df79d33b24ba10002e6ce37`
...
Two fresh runs per task on each build. The baseline passed 6/8; the final candidate passed 8/8. Neither final candidate set had a top-level tool error.
...
Raw local reports: `ux-baseline.json`, `ux-text-baseline.json`, `ux-reviewed-a.json`, and `ux-reviewed-b.json` in this directory. They contain synthetic traces and are ignored by Git. The final candidate ran as two independent four-case sets with identical recorded fingerprints.
...
succeeded with four tools and no errors in both runs. The final candidate
includes that inspection exception.
...
guide was not fetched in the final comparison runs, so their gains cannot be
