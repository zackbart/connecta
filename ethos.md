...
- **Operator routes manage authentication material, never declared
  capability.** Managing authentication for capabilities declared in deployment
  configuration is allowed; changing the connector set, tool catalog or
  annotations, requested OAuth scopes, admission policy, authorization rules,
  or caller tool scope is not — those require configuration and redeployment.
