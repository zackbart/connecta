...
Calls return plain values (JSON-parsing MCP text when possible) and throw; catch errors. Use connecta.batch for typed failures. Never retry retryable: false or rate_limited immediately — there are no timers. Return JSON; large results truncate, so reduce instead of returning raw payloads.
    ...
    "execute_code",
    {
      ...
      inputSchema: z.object({
        code: z
          .string()
          .describe(
            "One complete JavaScript async arrow function. Consume search/describe results and finish the task inside it; returning catalog data for a later call is invalid.",
          ),
        diagnostics: z
          .boolean()
          .optional()
