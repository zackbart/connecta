...
- connecta.search(args) and connecta.describe({ addresses: ["<connectorId>.<toolName>", ...] }) — load and inspect request-local catalogs on demand; describe takes an addresses array, never a single address string. Use safety: "readOnly" to avoid advertising calls this sandbox cannot execute; the filter changes results, not authority. Matches carrying schemas also list inputKeys, requiredInputKeys, and outputKeys — the same names the schema shows, ready to check against before building args. They are absent when a schema is not a plain object shape, so read the schema itself rather than assuming a missing list means no fields.
