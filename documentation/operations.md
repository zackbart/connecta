...
| `remote-mcp-credential.test.ts` | `remoteMcp()` drawing a static key from `/credentials`: the declared slot and its refusal of named fields, header name and framing (bearer, bare, and the two `Basic` forms) observed on the wire, an empty slot failing as `auth_required` rather than reaching the downstream, rotation replacing the cached client while a wiped value fails the next call, the Test action's catalog probe, the cleartext-destination warning, and the vault and `authorize_connector` handoff end to end |
