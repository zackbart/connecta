...
| `guest-api-contract.test.ts` | the executor-independent half of the guest API contract cases, plus the real Dynamic Worker authority boundary — local `data:` fetch; exact outbound denial for fetch, WebSocket, `node:net`, and `node:tls`; unresolved DNS; empty environment paths; unavailable filesystem and HTTP builtins; and the globals it leaves present |
