...
| `catalog.test.ts` | lexical ranking, conversational cleanup, and the compact schema renderer — `const`, `allOf` beside siblings, `$ref`, the depth limit, per-schema caching, and 2020-12 keyword compatibility |
...
| `meta-tools-search.test.ts` | registry-backed discovery: bounded search with page and address maxima, conversational-only refusal, weak one-term partial filtering, exact-name preservation, compact and JSON schemas with constraints, typed describe recovery and suggestions, and structured-result compatibility |
