...
`execute_code` before returning anything. It also names what a result value
means when the schema cannot: Mixpanel renders an absent boolean property as
`false` in a breakdown, so the guide says to confirm presence before reading
`false` as a signal
...
rendering rule beside it. *Cost:* result size, and a wrong conclusion drawn
from a right-looking value.
