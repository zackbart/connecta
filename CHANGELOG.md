...
counts for stores that implement the new optional `recordCatalogDrift`. A
deployment can ignore all of it: an unclassified tool already failed closed
onto `call_destructive_tool` before anyone counted it.
...
- **Hosted-provider drift detection at refresh.** Linear, Stripe, and Mixpanel
  ...
  payload-free event per change in the counts. The policy is written up in
